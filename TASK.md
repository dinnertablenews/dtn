...
8. The alternate. After the primary is chosen, pick a second story for the same slot: the highest-ranked remaining cluster whose category is **not** the primary's, and which passes rules 1, 2, and 6. For `evening`, the primary is the Good news story and the alternate is the highest remaining cluster of any other category. For `morning` and `noon`, if the primary is a hard story, prefer an alternate that is not. An alternate that does not run never counts as having run: it stays fair game for a later slot, and it does not count toward rule 2. Write which cluster it is and why in the log note.
...
- Append one entry to `data/log.json` for the slot: the primary's `category`, `headline`, `photo`, `cover_band`, `publish_target` (the drawn target), `chosen: "primary"`, and `alternate: {"folder": "posts/<slug>-alt", "category": ..., "headline": ..., "photo": true|false}`. Commit `posts/<slug>/`, `posts/<slug>-alt/`, and `data/log.json`, push to `main`.
- Write the summary for the notification, three lines: "Primary: <headline>", "Alternate: <headline>", and the target time ("Posts at 11:57 CT"). It reaches Dan's phone when this run's turn ends.
...
- Dan's replies wake this session. Handle each one, then end the turn again. Everything below is pushed to `main` before the target, because `main` is what the workflow reads:
  ...
  - **An edit** to either post: apply it, delete `caption`, re-render until exit 0, look at the JPGs, commit, push to `main`. The workflow pulls right before publishing, so a push that lands at least two minutes before the target is what goes out. Later than that: write HOLD, cancel the run, push the edit with HOLD removed, and dispatch again (omit `publish_at` if the target has passed).
