...
- `pip install feedparser requests pyyaml playwright pillow` if needed. Playwright's Chromium is preinstalled in this environment.

## 1. Candidates
- `python rank.py data/headlines.json 30 > /tmp/cand.json`. Read the top 15 clusters.
...
## 3. Facts
- Open the two or three source articles from the cluster (fetch the outlet links; skip any you can't reach). Write from what the articles say. No detail that isn't in them. Attribute the summary to the outlet whose article you leaned on most; that outlet's name goes in `outlet` and its URL in `source_url`.
...
- Cover: `headline` ≤ 60 characters (set `headline_size` 80 if it needs three lines); `summary` 3–5 sentences, ≤ 320 characters, ends before the outlet name (the template adds it).
...
- Dispatch the `Commons image` workflow with `subject` = the article title and `slug` = a kebab-case slug: `gh workflow run commons.yml -f subject="..." -f slug=...`. Wait ~90 s, `git pull`. If `data/images/<slug>.json` exists, set `photo` to its `file` and add its `credit` to the caption. If not, no photo; carry on.
...
- Schedule a wake-up for this same session in 50 minutes (send_later). When it fires: if Dan has replied in this conversation with "kill", "skip", or "hold", stop and log it. If he replied with a swap instruction, follow it (re-run §3–6 for the story he named), then publish. Otherwise publish:
  `gh workflow run publish.yml -f action=publish -f folder=posts/<slug>`
